---
name: go-coding
description: "Go 编码规范与最佳实践（编码规范类，与流程类 skill 不互斥）。涵盖惯用模式、注释规范、错误处理、并发、测试和安全编码。适用场景：(1) 用 Go 实现新需求或新功能，(2) 重构 Go 模块/包，(3) 对 Go 代码进行 code review，(4) 优化 Go 代码质量，(5) 其他 skill 驱动的流程涉及 Go 代码编写或修改时，作为编码规范补充加载。不适用于局部小修（语法修复、变量重命名、单行改动等）。"
---
...
本 skill 是**编码规范类**，定位为 Go 代码质量基线，与流程类 skill 不互斥。

| 模式 | 场景 | 行为 |
|------|------|------|
| 独立 | 用户直接要求 Go 编码指导、review 或重构 | 作为主 skill 加载 |
| 协同 | 其他 skill 驱动的流程涉及 Go 代码变更 | 与流程 skill 同时加载，约束代码质量 |
