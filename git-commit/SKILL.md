...
用户直接调用 skill / 表达出提交意图，或者 Agent 当前的 Edit 任务已经完成，进入收尾工作。先展示 commit message，再向用户明确提供后续选项并等待确认。

- 优先使用 `AskUserQuestion` 展示选项。
- 如果当前环境没有 `AskUserQuestion`，必须改用普通消息向用户提供同样的选项，并等待用户回复。
- 禁止在交互模式下替用户默认选择“仅提交”或其他动作；只有用户明确确认后才能执行。
...
如果 `AskUserQuestion` 不可用，使用普通消息给出同样三个选项，保持相同顺序，并等待用户明确回复后再执行。
...
用户确认后按选择执行。未收到明确确认前，不要执行 commit 或 push。
