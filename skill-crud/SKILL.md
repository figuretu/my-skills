...
| `scripts/skill-ops.sh check <name>` | 检查 skill 本地存在性 + 全局/仓库级别安装状态 |
| `scripts/skill-ops.sh install <name>` | 从本地仓库安装/重装 skill。Public skill：全局安装（按 `install-rules.json` 决定目标 agent）；Private skill：仓库级别安装（默认安装到 claude-code 和 codex，需在目标项目目录中执行） |
| `scripts/skill-ops.sh uninstall <name>` | 卸载 skill。Public skill：从所有默认 agent 全局卸载；Private skill：从当前仓库卸载 |
...
- **Private skill**：跳过此步。Private skill 按需在目标项目仓库中安装，不在 skill 仓库本身安装。提醒用户在目标项目目录中执行：

```bash
# 在目标项目目录中执行（使用 skill-crud 仓库中的脚本）
$MY_SKILLS_DIR/skill-crud/scripts/skill-ops.sh install <skill-name>
```
