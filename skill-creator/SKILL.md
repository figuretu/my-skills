...
To create a skill, follow the steps below in order, skipping steps only if there is a clear reason why they are not applicable.
...
### Step 3: Locate the my-skills Repository

Locate the my-skills repository where the new skill will be created. Try the following in order:

1. Attempt to find via zoxide:

```bash
zoxide query my-skills 2>/dev/null
```

2. If zoxide did not find it, check whether the environment variable `$MY_SKILLS_DIR` is set and the directory exists.

3. If neither method succeeds, inform the user that the my-skills repository cannot be found and ask for the path manually.
...
Update the skills table in `$MY_SKILLS_DIR/README.md` to include the new skill entry, keeping it consistent with the existing table format.
...
Install the new skill globally from the local repository so it becomes available across all projects:
...
5. Repeat Steps 6–8 (sync README, commit & push, reinstall) to apply changes
